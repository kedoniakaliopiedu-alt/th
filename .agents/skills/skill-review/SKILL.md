...
description: >
  ...
  the scripts under .Codex/skills/*/scripts/ and the lesson files under Thai A2/ and
...
- `{S}` = `.Codex/skills/skill-review` — the skill root.
- `{diff}` — the material under review, built in step 1.
- `{report}` = `.Codex/reviews/ревью-{цель}-{ГГГГ-ММ-ДД}.md` — where findings land.
- `{deferred}` = `.Codex/reviews/отложено.md` — the running list of parked findings.
...
2. Load `.Codex/skills/README.md` — the map of skills, what each owns, and how they hand
   ...
   map.
3. Load `AGENTS.md` — the project's non-negotiable rules (images always go through
   `thai-handwriting`; Cyrillic-only transcription; tones verified against an authoritative
   source; progress written through `tracker.py`). A change that contradicts `AGENTS.md` is
