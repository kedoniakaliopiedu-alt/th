...
def to_png(path: Path) -> Path:
    ...
    try:
        ...
        raise OcrFailed("нет утилиты sips — конвертировать нечем") from None
    except subprocess.CalledProcessError as e:
        detail = (e.stderr or b"").decode("utf-8", "replace").strip()
        raise OcrFailed("sips не преобразовал файл{}".format(
...
def line_index(path: Path) -> int | None:
    ...
    В цикл попадает только `line-*.png`, но и среди них встречается файл, названный
    не по счёту (`line-crop.png`, `line-fix.png`). Раньше такой ронял всё сравнение
    на `int()`.
    """
...
def ocr(path: Path, timeout: int = 300) -> str:
    ...
    try:
        ...
        # Таймаут — свойство конкретной картинки (длинная строка, тяжёлая модель), а
        # не всего прогона: следующая вполне может прочитаться. Всё остальное здесь —
        # «соединения нет», и тогда продолжать действительно незачем.
        if isinstance(e.reason, (TimeoutError, socket.timeout)):
            raise OcrFailed(f"не уложился в {timeout} с") from e
        sys.exit(f"typhoon: ollama недоступен на {HOST} — запусти `ollama serve` ({e})")
    except TimeoutError as e:
        # Таймаут при чтении тела: URLError его не оборачивает.
        raise OcrFailed(f"не уложился в {timeout} с при чтении ответа") from e
    except OSError as e:
        raise OcrFailed(f"обрыв связи с ollama ({e})") from e
    try:
...
def main() -> None:
    ...
    if args.compare or args.dir:
        ...
        # calibration.md: Typhoon — модель уровня страницы, на узких полосках
        # галлюцинирует. Режим оставлен для сопоставления с Vision, но предупредить
        # надо: иначе прочитанное здесь легко принять за независимое подтверждение.
        print("typhoon: это построчный режим, а модель уровня страницы — на узких "
              "полосках она склонна выдумывать (references/calibration.md). Для "
              "чтения давай ей страницу целиком.", file=sys.stderr)
        failed: list[str] = []
        for f in files:
            idx = line_index(f)
            try:
