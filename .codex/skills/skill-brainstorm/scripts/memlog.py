...
Adapted from BMAD Method (src/scripts/memlog.py, MIT) — see ATTRIBUTION.md.
"""
...
def read_log(path: Path) -> "tuple[dict[str, str], str] | None":
    """Read an existing memlog, or explain why it cannot be read and return None.

    `init` answers its own failure with a sentence and exit code 2; append/set used to
    answer theirs with a traceback. Same shape of problem, same shape of answer.
    """
    try:
        ...
        print(f"error: {path} could not be read ({e})", file=sys.stderr)
    except ValueError as e:
        print(f"error: {path} is not a memlog ({e})", file=sys.stderr)
    return None
