...
> `.codex/skills/MAP.md` (the map of skills and their zones) and `AGENTS.md` (project rules
> that override everything else), then review the material.
>
> Look for:
> - **a violation of a `AGENTS.md` rule** — e.g. reading Thai off an image bypassing
...
> `.codex/skills/thai-tasks/SKILL.md`, section «Три опоры»: a task is an action in context,
...
> `.codex/skills/*/scripts/` — currently `tracker.py`, `thaiocr` (+`thaiocr.swift`),
