...
- a gap in a `SKILL.md` may be filled by its `references/` file — the top-level file is
  ...
  finding;
- a behaviour that looks unspecified may be mandated by `AGENTS.md` for the whole project;
...
— see `AGENTS.md`, «Служебный вывод». No `echo` banners, no byte counts, no before/after
...
- `high` — **intolerable.** A lesson goes wrong: wrong Thai reaches her as correct, a
  `AGENTS.md` rule is broken, progress data is corrupted or lost, a mandated step can be
