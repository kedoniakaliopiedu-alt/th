...
Write `{report}` = `.codex/reviews/ревью-{target}-{ГГГГ-ММ-ДД}.md` (create
`.codex/reviews/` if missing). If a file with that name exists, append a new
...
Also append each `отложить` finding to `{deferred}` = `.codex/reviews/отложено.md`, under
