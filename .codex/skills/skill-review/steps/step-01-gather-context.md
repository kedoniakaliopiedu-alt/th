...
- **A skill name** («аудит thai-tasks», «отревьюй skill-writer») → `{scope}` = `skill`,
  `{target}` = that skill. The material is every file under `.codex/skills/<name>/`.
...
**Tier 3 — a change list from a brainstorm.** Glob `.codex/brainstorms/*/изменения.md`.
If exactly one exists and is newer than the last review in `.codex/reviews/`, suggest it:
...
- `scope` = `skill` → `{paths}` = every file under `.codex/skills/<target>/` (Glob, do not
...
- `AGENTS.md` — project rules that override everything;
- `.codex/skills/MAP.md` — skill boundaries and hand-offs;
