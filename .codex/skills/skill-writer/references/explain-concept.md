...
- **Name the file, with its full path.** Every claim should be traceable to a place she can
  open: `.codex/skills/thai-tasks/references/progress-and-spiral.md` says X;
  `.codex/skills/thai-tasks/scripts/tracker.py` implements Y. A bare `references/…` is
...
1. **Locate the mechanism** — which skill owns it, which reference file holds the full
   version, which script implements it. `.codex/skills/MAP.md` is the map.
