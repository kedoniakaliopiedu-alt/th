...
Единственный скрипт репозитория, который ходит в сеть, и это разрешённое исключение из
правила «скрипты офлайн»: сеть тратится ровно на то, чтобы занятие перестало от неё зависеть
(условия исключения — skill-writer/references/skill-doc-standards.md, раздел «Scripts»).
