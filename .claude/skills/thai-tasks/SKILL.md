...
главе. Все правила самой контрольной — в `thai-learning` (раздел «Система контроля»):
масштабы и пороги (мини-контроль 80%, итоговый 90%), **запрет подсказок**, оценка в
процентах, шапка `🔒`, «не сдал → тренировка → пересдача новыми вопросами». **Не дублируй
их здесь** — открой thai-learning и следуй ему.

`thai-tasks` в срезе отвечает за две свои вещи:

- **Сборка на покрытие, а не спираль 60/40.** Дефолт «60% нового» здесь не действует. Цель
  среза — пропорционально покрыть **всю** проверяемую тему/главу, а не двигать новое.
  Собери выборку так: пройди по всем подтемам скоупа + через **get_due** подними в
  приоритет просроченные и слабые элементы (низкий mastery, где были ошибки). Разные типы
  заданий (лексика / тоны / орфография / грамматика / продукция) — по каталогу, чтобы срез
  мерил навык целиком, а не один формат.
- **Возврат результатов в трекер — обязателен.** Срез даёт самый сильный сигнал для SM-2.
  После проверки прогони `scripts/tracker.py record` по **каждому** элементу с реальным
  исходом (не только по ошибкам), занеси паттерны через `mistake` и прокинь итоговый
  процент в `set-meta --recent-accuracy`. Затем — `thai-mistakes` на отработку.

Оформление — как воркшит (буквенные блоки, обнуляемая нумерация, чистый лист), но **без
подсказок и без разбора внутри листа**: разбор идёт единым отчётом после всех ответов.
Значок ⚑ на ещё не объяснённые слова ставится и в контрольной (это справка, не подсказка).
