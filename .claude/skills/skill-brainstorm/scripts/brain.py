...
def pretty(cat: str) -> str:
    """Отображаемое имя категории: русский ярлык из CATEGORY_LABELS, иначе slug как есть."""
    return CATEGORY_LABELS.get(cat, cat.replace("_", " ").replace("-", " ").title())


# Русские имена категорий каталога. Категория, которой тут нет (например добавленная
# через --extra), показывается своим slug'ом — так кастомные каталоги всегда рендерятся.
CATEGORY_LABELS = {
    "structured": "Структурные",
    "deep": "Глубокие",
    "creative": "Творческие",
    "biomimetic": "Биомиметика",
    "cultural": "Культурные",
    "speculative_future": "Спекулятивное будущее",
    "quantum": "Квантовые",
    "wild": "Дикие",
    "absurdist": "Абсурдистские",
    "theatrical": "Театральные",
    "constraint": "Ограничения",
    "introspective_delight": "Интроспективные",
...
<title>Композер сессии мозгового штурма</title>
<script>
/* set the theme before first paint so there's no light-mode flash */
(function(){ try {
  var t = localStorage.getItem('skillkit-theme');
...
<header>
  <div class="hwrap">
  <div class="titlerow">
    <h1>Композер сессии мозгового штурма</h1>
    <button id="theme" class="themebtn" type="button" aria-label="Тёмная тема" title="Тёмная тема"></button>
  </div>
  <p class="sub">Собери сессию, нажми <strong>Скопировать промпт</strong> и вставь результат обратно в чат. {{TOTAL}}</p>

  <div class="composer">
    <div class="grp">
      <span class="glabel">Ведение</span>
      <div class="modes" id="modes">
        <button type="button" class="mode on" data-mode="Фасилитатор">Фасилитатор</button>
        <button type="button" class="mode" data-mode="Со-творец">Со-творец</button>
        <button type="button" class="mode" data-mode="Придумай за меня">Придумай за меня</button>
      </div>
      <span class="modehint" id="modehint"></span>
    </div>
    <div class="grp">
      <span class="glabel">Техники</span>
      <span class="pill">Выбрано <b id="pickN">0</b></span>
      <span class="step">Случайно <button type="button" data-step="rand" data-d="-1">&minus;</button><b id="randN">0</b><button type="button" data-step="rand" data-d="1">+</button></span>
      <span class="step">Придумать <button type="button" data-step="inv" data-d="-1">&minus;</button><b id="invN">0</b><button type="button" data-step="inv" data-d="1">+</button></span>
      <span class="step">Выберет Claude <button type="button" data-step="ai" data-d="-1">&minus;</button><b id="aiN">0</b><button type="button" data-step="ai" data-d="1">+</button></span>
      <span class="total" id="total">Всего 0 &middot; 3&ndash;4 — золотая середина</span>
      <button id="copy" type="button">Скопировать промпт</button>
    </div>
  </div>

  {{GOALBAR}}
  <div class="bar">
    <span class="glabel">Перейти к</span>
    <div class="chips" id="chips">{{CHIPS}}</div>
  </div>

  <div class="banner" id="banner">&#10003; Скопировано! Вставь в чат, чтобы начать сессию.</div>
  </div>
</header>
<main>
{{BODY}}
</main>
<footer>skill-brainstorm &middot; адаптация BMAD Method (MIT)</footer>
<script>
(function(){
  ...
  var state = { mode: 'Фасилитатор', rand: 0, inv: 0, ai: 0 };
  var MODE_HINTS = {
    'Фасилитатор': 'Выжимаю идеи из тебя: спрашиваю, давлю, ставлю рамки — но своих идей не подкидываю.',
    'Со-творец': 'Работаем в паре — я и веду, и подбрасываю свои идеи; в логе помечено, чья какая.',
    'Придумай за меня': 'Провожу всю сессию сам, показываю результат и предлагаю продолжить вместе.'
  };
  ...
  function update(){
    ...
    var t = $('total');
    t.textContent = 'Всего ' + total + ' · 3–4 — золотая середина';
  ...
  function compose(){
    ...
    var L = ['Запускаем сессию мозгового штурма.', "", 'Режим ведения: ' + state.mode + '.'];
    if (techs.length){
      L.push("", 'Техники:');
      techs.forEach(function(t, i){
        L.push((i + 1) + '.' + (t.r ? ' (случайный выбор)' : '') + ' ' + t.n + '  ·  ' + t.c);
        L.push('   ' + t.d);
      });
    }
    var extra = [];
    if (state.inv > 0){ extra.push('придумай ' + state.inv + ' новых техник на ходу'); }
    checkedInvent().forEach(function(b){ extra.push('придумай 1 новую технику в духе категории «' + b.dataset.invent + '»'); });
    if (state.ai > 0){ extra.push('выбери сам ещё ' + state.ai + ' техник под мою цель'); }
    if (extra.length){ L.push("", 'Затем: ' + extra.join('; и ') + '.'); }
    if (!techs.length && !extra.length){
      L.push("", state.mode === 'Придумай за меня'
        ? 'Проведи всю сессию сам — выбери техники, нагенерируй идей и покажи мне результат.'
        : 'Помоги выбрать 3–4 техники для старта.');
    }
    return L.join('\n');
  }
...
CLASSIC_GROUP = "Проверенные и профессиональные"
...
CATEGORY_GROUPS = (
    ("Методика обучения", ("методика",)),
    ("Структура и анализ", ("structured", "deep")),
    ("Творчество и генерация", ("creative", "biomimetic", "cultural", "speculative_future", "quantum")),
    ("Дикие и игровые", ("wild", "absurdist", "theatrical", "constraint")),
    ("Интроспекция и совместные", ("introspective_delight", "collaborative")),
)

# Human labels for the `good_for` goal tags; this dict's order is the filter-bar order.
GOAL_LABELS = {
    "feature": "Собрать механику",
    "novel": "Новая идея",
    "strategy": "Стратегия",
    "planning": "Планирование",
    "diagnosis": "Диагностика",
    "personal": "Личное",
    "unstuck": "Расшевелиться",
}


def _good_for_label(good: str) -> str:
    parts = [GOAL_LABELS.get(g, g) for g in good.split("|") if g]
    return ("Хорошо для: " + " · ".join(parts)) if parts else ""
...
def html_doc(rows: list[Row]) -> str:
    ...
    if leftover:
        body.append('<h2 class="grouphdr" style="--c:#8a8f9e">Прочее</h2>')
        for c in leftover:
            add_section(c)
    ...
    if present_goals:
        ...
        goalbar = f'<div class="bar"><span class="glabel">Хорошо для</span><div class="goals" id="goals">{gchips}</div></div>'
